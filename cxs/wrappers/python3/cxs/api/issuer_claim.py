...
class IssuerClaim(CxsStateful):
    ...
    async def update_state(self) -> int:
        return await self._update_state(IssuerClaim, 'cxs_issuer_claim_update_state')
    ...
    async def send_offer(self, connection: Connection):
        if not hasattr(IssuerClaim.send_offer, "cb"):
            self.logger.debug("cxs_issuer_send_claim_offer: Creating callback")
    ...
    async def send_claim(self, connection: Connection):
        if not hasattr(IssuerClaim.send_claim, "cb"):
            self.logger.debug("cxs_issuer_send_claim: Creating callback")
