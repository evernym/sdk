...
mod tests {
    ...
    fn test_cxs_create_schema_and_create_claimdef_with_pool() {
        settings::set_defaults();
        open_sandbox_pool();
        let wallet_handle = init_wallet("my_real_wallet").unwrap();
        let (my_did, my_vk) = SignusUtils::create_and_store_my_did(wallet_handle, Some("000000000000000000000000Anchor00")).unwrap();
        settings::set_config_value(settings::CONFIG_ENTERPRISE_DID, &my_did);
