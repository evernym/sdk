...
lazy_static! {
    static ref CLAIMDEF_MAP: Mutex<HashMap<u32, Box<ClaimDefinition>>> = Default::default();
}
...
impl ClaimDefinition {
    ...
    pub fn from_str(claimdef:&str) -> Result<ClaimDefinition, u32> {
        match serde_json::from_str(claimdef) {
            Ok(p) => Ok(p),
            Err(err) => {
                warn!("{} with serde error: {}",error::CREATE_CLAIM_DEF_ERR.message, err);
                Err(error::CREATE_CLAIM_DEF_ERR.code_num)
            },
        }
    }

    pub fn retrieve_claim_def(&mut self,
                              submitter_did: &str,
                              schema_num:u32,
                              signature_type:&str,
                              issuer_did: &str) -> Result<String, u32> {
        self.issuer_did = String::from(issuer_did);
        self.schema_seq_no = schema_num;
        self.signature_type = String::from(signature_type);

        let request = self.build_get_txn(submitter_did)?;
    ...
    pub fn build_get_txn(&self, submitter_did: &str) -> Result<String, u32> {
        ...
        unsafe {
            let indy_err = indy_build_get_claim_def_txn(command_handle,
                                                        ...
                                                        CString::new( self.signature_type.as_str()).unwrap().as_ptr(),
                                                        CString::new(self.issuer_did.as_str()).unwrap().as_ptr(),
                                                        cb);
            if indy_err != 0 {
    ...
    pub fn build_create_txn(&self, claim_def_json: &str) -> Result<String, u32> {
        ...
        unsafe {
            let indy_err = indy_build_claim_def_txn(command_handle,
                                                        ...
                                                        self.schema_seq_no as i32,
                                                        CString::new( self.signature_type.as_str()).unwrap().as_ptr(),
                                                        CString::new(claim_def_json).unwrap().as_ptr(),
                                                        cb);
            if indy_err != 0 {
                return Err(error::CREATE_CLAIM_DEF_ERR.code_num)
            }
        }
        ...
        info!("Created claim_def transaction");
        Ok(claim_def_req)
    }
...
pub fn create_new_claimdef(claimdef_name: String,
    ...
    let claim_def_json = create_and_store_claim_def(&schema_data, &issuer_did,  "CL",create_non_revoc)?;
    let mut new_claimdef = Box::new(ClaimDefinition::from_str(&claim_def_json)?);
    //Todo: Store claim def on ledger
    let claim_def_txn = new_claimdef.build_create_txn(&new_claimdef.data.to_string()?)?;
...
fn create_and_store_claim_def(schema_json: &str,
    ...
    let (command_handle, cb) = CallbackUtils::closure_to_build_request_cb(cb);
    unsafe {
        let indy_err = indy_issuer_create_and_store_claim_def(command_handle,
                                                              wallet_handle,
                                                              CString::new(issuer_did).unwrap().as_ptr(),
                                                              CString::new(schema_json).unwrap().as_ptr(),
    ...
    let (err, claim_def_json) = receiver.recv_timeout(TimeoutUtils::long_timeout()).unwrap();

    if err != 0{
        return Err(error::CREATE_CLAIM_DEF_ERR.code_num)
    }
    info!("Created claim_def and stored it in wallet");
    Ok(claim_def_json)
}
...
pub mod tests {
    ...
    static ISSUER_DID: &str = "2hoqvcwupRTUNkXn6ArYzs";
    ...
    fn test_get_claim_def_request() {
        ...
        let mut claim_def = ClaimDefinition::new();
        claim_def.schema_seq_no = 15;
        claim_def.issuer_did = "4fUDR9R7fjwELRvH9JT6HH".to_string();

        let claim_def_req = claim_def.build_get_txn("GGBDg1j8bsKmr4h5T9XqYf").unwrap();
    ...
    fn test_get_claim_def_by_send_request_fails() {
        settings::set_defaults();
        assert_eq!(ClaimDefinition::new().send_request("{}"), Err(error::NO_POOL_OPEN.code_num));
    }

    #[test]
    fn test_build_claim_def_req_fails() {
        assert_eq!(ClaimDefinition::new().build_get_txn(""), Err(error::BUILD_CLAIM_DEF_REQ_ERR.code_num));
    }
...
//        assert!(init_wallet(&settings::CONFIG_WALLET_NAME).unwrap() > 0);
//        let wallet_handle = get_wallet_handle();
//        let claim_def = ClaimDefinition::new().retrieve_claim_def("GGBDg1j8bsKmr4h5T9XqYf",
//                                                  15,
//                                                  "CL",
...
//    }
    ...
    fn test_create_claim_def_and_store_in_wallet() {
        ...
        assert!(init_wallet(&settings::CONFIG_WALLET_NAME).unwrap() > 0);
        let wallet_handle = get_wallet_handle();
        let claim_def_json = create_and_store_claim_def(SCHEMAS_JSON, ISSUER_DID, "CL", false).unwrap();
    ...
    fn test_create_claim_def_txn_and_submit_req() {
        ...
        println!("\nMyDid: {:?}\n", my_did);
        println!("\nMyVerkey: {:?}\n", my_vk);
        let claim_def_json = create_and_store_claim_def(SCHEMAS_JSON, &my_did, "CL", false).unwrap();
//        let claim_def_json = create_and_store_claim_def(SCHEMAS_JSON, ISSUER_DID, "CL", false).unwrap();
        let claim_def_obj = ClaimDefinition::from_str(&claim_def_json).unwrap();
        let claimdef_data = serde_json::to_string(&claim_def_obj.data).unwrap();
        let claim_def_txn = claim_def_obj.build_create_txn(&claimdef_data).unwrap();
        println!("ClaimDef TXN:  {:?}", claim_def_txn);
        let claim_def_result = claim_def_obj.sign_and_send_request(&claim_def_txn).unwrap();
