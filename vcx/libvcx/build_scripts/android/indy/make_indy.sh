...
if [ "${FINAL}" == "1" ] ; then
    ...
    $CC -shared -o libindy.so -Wl,--whole-archive ${TOOLCHAIN_DIR}/sysroot/usr/lib/libstdc++.a ${TOOLCHAIN_DIR}/sysroot/usr/lib/libz.a ${TOOLCHAIN_DIR}/sysroot/usr/lib/libm.a indy-sdk/libindy/target/${CROSS_COMPILE}/release/libindy.a ${OPENSSL_DIR}/lib/libssl.a ${OPENSSL_DIR}/lib/libcrypto.a ${SODIUM_LIB_DIR}/libsodium.a ${LIBZMQ_DIR}/lib/libzmq.a -Wl,--no-whole-archive -z muldef
else
    cargo build --release --target=${CROSS_COMPILE}
