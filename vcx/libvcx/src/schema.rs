...
pub trait Schema: ToString {
    ...
    fn process_ledger_txn(txn: String) -> Result<SchemaTransaction, SchemaError>
    {
        ...
        match schema_txn.clone().txn_type {
            ...
            None => Err(SchemaError::CommonError(error::INVALID_SCHEMA_SEQ_NO.code_num))
        }
    }
...
impl CreateSchema {
    pub fn create_schema_req(submitter_did: &str, data: String) -> Result<String, SchemaError> {
        if settings::test_indy_mode_enabled() { return Ok(SCHEMA_REQ.to_string()); }
        libindy_build_schema_request(submitter_did.to_string(), data).or(Err(SchemaError::InvalidSchemaCreation()))
//            .map_err( |x| error::map_libindy_err(x, error::INVALID_SCHEMA_CREATION.code_num))
    ...
    pub fn sign_and_send_request(submitter_did: &str, request: &str) ->  Result<String, SchemaError> {
        ...
        libindy_sign_and_submit_request(pool_handle,
                                        ...
                                        request.to_string()).or(Err(SchemaError::InvalidSchemaCreation()))
    }
    ...
    static DIRTY_EXAMPLE: &str = r#"
{
  ...
  "type":"101"
    ...
    fn test_process_ledger_txn(){
        ...
        let bad_ledger_schema = LedgerSchema::process_ledger_txn(
            String::from_str(BAD_LEDGER_SAMPLE).unwrap());
